...
const checkBoard = (board, word, x, y, checked) => {
  ...
  if (x > 0 && checkBoard(board, word.slice(1), x-1, y, checked)) {
    return true;
  } else if (x < 4 && checkBoard(board, word.slice(1), x+1, y, checked)) {
    return true;
  } else if (y > 0 && checkBoard(board, word.slice(1), x, y-1, checked)) {
    return true;
  } else if (y < 0 && checkBoard(board, word.slice(1), x, y+1, checked)) {
    return true;
  } else {
...
function find(board, word) {
  ...
  for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
      if (checkBoard(board, word, i, j, new Set())) return true
    }
  }
